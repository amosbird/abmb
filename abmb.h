...
template <typename Resolution = std::chrono::nanoseconds, std::size_t iter = 1,
          std::size_t run = 100, bool timePerIter = true, typename TFunc>
inline __attribute__((always_inline)) double mb(TFunc&& func) {
    return mbs<Resolution, iter, run, timePerIter, TFunc>(std::forward<TFunc>(func)).avg();
}
